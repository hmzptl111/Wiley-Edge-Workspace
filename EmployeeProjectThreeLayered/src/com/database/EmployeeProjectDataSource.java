...
public class EmployeeProjectDataSource {
	...
	static {
		for(Project project: ProjectDataSource.getProjects().get()) {
	...
	public static boolean assignEmployeeToProjectById(int eId, int pId) {
		...
		Optional<HashSet<Employee>> employeesAssignedToProject = Optional.ofNullable(getEmployeesAssignedToProjectById(pId).orElse(new HashSet<Employee>()));
